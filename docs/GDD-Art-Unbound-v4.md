...
El menú principal es un panel curvo en espacio MR/VR, dividido en zonas:
...
- Muestra miniatura de cada obra. Las obras ya completadas muestran una **medalla de completación única** (indica que la obra fue armada; no varía según el tamaño elegido). Las obras en progreso muestran su porcentaje de avance.
...
- Ofrece tres botones de tamaño para iniciar el puzzle, con el tiempo récord alcanzado en cada uno.
- Este panel está enfocado únicamente en **jugar**. La acción de colgar obras vive en la sección Collection (ver abajo), no aquí.

**Collection (sección de exhibición)**
- Inventario único de todo lo que el jugador puede exhibir en su espacio: **obras completadas**, placas temáticas, placa de estatus, lámpara y cédula.
- **Solo aparecen obras completadas** (no se puede colgar lo que no se ha armado). El catálogo del panel central sigue mostrando todas las obras para jugar; una obra completada vive en ambos lugares con intención distinta: catálogo = jugar, Collection = exhibir.
- Al seleccionar una obra completada o una placa, el jugador entra **directo al modo de colocación** para colgarla (ver secciones 5 y 6), sin pasar por un panel de detalle: colgar no requiere ninguna decisión, ya que el marco es constante y el cuadro es idéntico sin importar el tamaño en que se armó.
- Al seleccionar un objeto cosmético **bloqueado** (placa, lámpara o cédula aún no obtenida), una ventana indica el logro o condición necesaria para desbloquearlo.

### 4.2. Tamaños de Rompecabezas

El jugador elige entre tres tamaños, planteados como **compromisos de tiempo** según cuánto desee permanecer en una sesión, no como niveles de dificultad competitiva.
...
| An Afternoon | 4 cm | ~120–225 | ~1–2 h |
...
- Botón "¿Cuáles están mal?" (ver sección 4.6).
...
### 4.6. Resaltar Piezas Incorrectas
...
1. Al colocar todas las piezas correctamente se activa la animación de revelación: el cuadro completo aparece con su marco.
2. Se muestra el panel de post-juego con el tiempo de resolución.
3. Se actualiza el progreso de logros del jugador (ver sección 9) y, si corresponde, se notifican los logros recién desbloqueados.
4. El juego presenta la opción de colgar el cuadro en una pared.
...
El colgado puede iniciarse desde dos puntos: el panel de post-juego (justo al terminar una obra) o la sección Collection del menú (para colgar cualquier obra ya completada o placa obtenida). Ambos llevan al mismo flujo:

1. El usuario toma el cuadro completado con un gesto de pinch.
...
El mismo flujo de colgado aplica a las **placas temáticas** y a la **placa de estatus** (ver sección 8): son objetos colgables que el usuario coloca libremente donde desee.

### 5.2. Reposicionamiento y Retiro

- Los cuadros y placas ya colgados pueden tomarse de nuevo con pinch.
- Al soltar cerca de una pared (≤ 20 cm): el objeto se reposiciona y el anclaje espacial se actualiza.
- Al soltar lejos de una pared: el objeto se retira de la pared y desaparece.
...
En modo VR el usuario se encuentra dentro de una galería de arte 3D inmersiva. El puzzle se arma en el mismo lienzo flotante que en MR, pero la interacción se basa en **apuntar y disparar con el controlador** en lugar de gestos de pinch. Las paredes de la galería son superficies virtuales donde puede colgar sus obras completadas, placas y cédulas. A diferencia de MR, la galería virtual no tiene límite de espacio físico: el usuario puede exhibir su colección completa en múltiples paredes.
...
Igual que en MR, el colgado puede iniciarse desde el post-juego o desde la sección Collection del menú. El flujo:
...
Panel curvo en espacio MR/VR (ver sección 4.1):
- **Izquierda**: Configuración de audio y canción en reproducción.
- **Centro**: Galería con cuadrícula paginada, filtros (Todas / En Progreso / Completadas) y medalla de completación única en obras armadas.
- **Derecha**: Detalle de obra — pintura grande, título, artista, museo, descripción y selección de tamaño con récords (solo para jugar).
- **Collection**: inventario de exhibición — obras completadas, placas, lámpara y cédula; colgado directo de obras/placas y descubrimiento de objetos bloqueados con sus condiciones.
...
Un único producto de compra dentro de la app, **"Desbloquear catálogo completo"**, a **$9.99 USD**, otorga acceso permanente a las **240+ obras restantes** del catálogo. Es una compra única, no una suscripción.

- No existen packs temáticos individuales ni bundles como productos de tienda. El desbloqueo es total y único.
- Todas las obras desbloqueadas quedan disponibles de inmediato, sin progresión intermedia que limite su acceso.
